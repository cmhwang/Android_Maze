...
//	public CardinalDirection currentDirection;
	...
	// may not need some of above var
	// RELIANT ON WHETHER CONTROLLER BEING SET UP AND THEN ROBOT BEING ASSIGNED TO IT - call it once
	
	
	public ReliableSensor forwardSensor;
	public ReliableSensor leftSensor;
	public ReliableSensor rightSensor;
	public ReliableSensor backSensor;
	
	public String robotType = "reliable";
	 ...
	 */
	@Override
	public void setController(StatePlayingAnimated c) {
		// part of class coordination: specifies to the robot what control object to work with
		...
		} else if (c.getState().getMaze() == null) {
			throw new IllegalArgumentException();
		} else {
			controller = c;
		}
	...
	public void addDistanceSensor(DistanceSensor sensor, Direction mountedDirection) {
		...
		// will always have 2 distance sensors for foward, left - if turn made will use this method to add right or back
		// ex: if facing forward wall will use distanceToObstacle method to return the right distance
		// how it works in code: will check if alreay mounted (will override) or will add in that direction
		...
		if (sensor == null || mountedDirection == null) {
			...
			forwardSensor = (ReliableSensor) sensor;
	...
	public int[] getCurrentPosition() throws Exception {
		...
		int[] toReturn = controller.getState().getCurrentPosition();
		
		if (!((toReturn[0] >= 0) && (toReturn[0] < controller.getState().getMaze().getWidth()) && (toReturn[1] >= 0) && (toReturn[1] < controller.getState().getMaze().getHeight()))) {
			throw new Exception();
		} else {
			return toReturn;
		}
	...
	public CardinalDirection getCurrentDirection() {
		...
		return controller.getState().getCurrentDirection();
	}
	...
	public void rotate(Turn turn) {
		...
		if (turn.equals(Turn.LEFT)) {
			...
			if (energyLevel <= 0) {
				...
				controller.getState().handleUserInput(UserInput.LEFT, 0);
			}		
			...
			if (energyLevel <= 0) {
				...
				controller.getState().handleUserInput(UserInput.RIGHT, 0);
			}
			
		} else {
			assert turn.equals(Turn.AROUND) : "Last remaining turn option";
	 ...
	 * special cases: if forward sensor down for repair then wait
	...
	public void move(int distance) {
		...
		// added part in p4 to let process wait if sensor down for reapir
		...
		if (distance < -1) {
			...
			int distanceToWall = distanceToObstacle(Direction.LEFT);
			if (distanceToWall == 0) {
				isDead = true;
			} else {
				...
				if (energyLevel <= 0) { // CHECK ON THIS
					...
					controller.getState().handleUserInput(UserInput.LEFT, 0);
					controller.getState().handleUserInput(UserInput.UP, 0);
			...
			try {
				if (forwardSensor.checkRepairStatus()) {// sleeps if being repaired
					try {
						Thread.sleep(2000);
		...
		if (energyLevel <= 0) { // case for if energy dies
			...
			try {
				...
				if (curPos[1] == 0 && getCurrentDirection() == CardinalDirection.North) {// cases for if jump over border
					...
					isDead = true;
				} else if (curPos[1] == (controller.getState().getMaze().getHeight() - 1) && getCurrentDirection() == CardinalDirection.South) {// cases for if jump over border
					isDead = true;
				} else if (curPos[0] == (controller.getState().getMaze().getWidth() - 1) && getCurrentDirection() == CardinalDirection.East) {// cases for if jump over border
					isDead = true;
				} else {
					
					if (getCurrentDirection() == CardinalDirection.North) {
						controller.getState().handleUserInput(UserInput.JUMP, 0);
						odometerReading = odometerReading + 1;
	...
	public boolean isAtExit() {
		...
		if (controller.getState().getCurrentPosition() == controller.getState().getMaze().getExitPosition()) {
			return true;
		} else {
			return false;
		}
	...
	public boolean isInsideRoom() {
		...
		int[] position = controller.getState().getCurrentPosition();
		
		if (controller.getState().getMaze().isInRoom(position[0], position[1])) {
			return true;
		} else {
			return false;
		}
	 ...
	 * special case: pauses process if needed distance sensor down for repair
	...
	public int distanceToObstacle(Direction direction) throws UnsupportedOperationException {
		...
		// added part for p4 to pause process if repair for needed sensor ongoing
		// special case: gives max integer val if facing exit, calls on canSeeThroughExit()
		...
		CardinalDirection curCarDir = getCurrentDirection();
			...
			if (forwardSensor == null) {
				...
				try {
					if (canSeeThroughTheExitIntoEternity(Direction.FORWARD)) {
						...
						toReturn = forwardSensor.distanceToObstacle(curPos, curCarDir, energyRep);
					}
				} catch (Exception e) {
					e.printStackTrace();
					toReturn = -100;
			...
			if (leftSensor == null) {
				...
				try {
					if (canSeeThroughTheExitIntoEternity(Direction.LEFT)) {
						...
						toReturn = leftSensor.distanceToObstacle(curPos, inputCarDir, energyRep);
					}
				} catch (Exception e) {
					e.printStackTrace();
					toReturn = -100;
			...
			if (backSensor == null) {
				...
				try {
					if (canSeeThroughTheExitIntoEternity(Direction.BACKWARD)) {
						...
						toReturn = backSensor.distanceToObstacle(curPos, inputCarDir, energyRep);
					}			
				} catch (Exception e) {
					e.printStackTrace();
					toReturn = -100;
			...
			if (rightSensor == null) {
				...
				try {
					if (canSeeThroughTheExitIntoEternity(Direction.RIGHT)) {
						...
						toReturn = rightSensor.distanceToObstacle(curPos, inputCarDir, energyRep);
					}
				} catch (Exception e) {
					e.printStackTrace();
					toReturn = -100;
	 ...
	 * how it will be used; distanceToObstacle calls it to help out with seeing if it is facing an exit, also sleeps temporarily if sensor is down for repair
	...
	public boolean canSeeThroughTheExitIntoEternity(Direction direction) throws UnsupportedOperationException {
		...
		// added part in project 4 to wait if sensor being repaired
		...
		if (direction == Direction.FORWARD) {
			if (forwardSensor == null) {
				...
				if (forwardSensor.checkRepairStatus()) {// checks if forward sensor down for repair
					try {
						Thread.sleep(2000);
					} catch (InterruptedException e) {
						e.printStackTrace();
			...
			if (leftSensor == null) {
				...
				if (leftSensor.checkRepairStatus()) {//check if left sensor down for repair
					try {
						Thread.sleep(2000);
					} catch (InterruptedException e) {
						e.printStackTrace();
			...
			if (backSensor == null) {
				...
				if (backSensor.checkRepairStatus()) {//check if back sensor down for repair before using
					try {
						Thread.sleep(2000);
					} catch (InterruptedException e) {
						e.printStackTrace();
			...
			if (rightSensor == null) {
				...
				if (rightSensor.checkRepairStatus()) {// check if right sensor down for repair before using
					try {
						Thread.sleep(2000);
					} catch (InterruptedException e) {
						e.printStackTrace();
	...
	/**
	 * this method is not supposed to be invoked on reliable robot so just throws exception
	 * exception automatically since not supposed to be called
	 */
	@Override
